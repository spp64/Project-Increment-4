 ...
 * 
 * Added touch control Added view score and record Added menu walls on/off Added
 * menu big,normal,small size Added menu about Added save preferences Added
 * vibration Added red pepper, slow down the snake Added records dialog Added
 * alternate input turn mode Added fast speed and more
 */
...
public class MSnake extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Date date = new Date();
		gameTime = dateFormat.format(date);
		start = System.currentTimeMillis();
		storeValues = (Button) findViewById(R.id.saveData);
		storeValues.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				int count = SnakeView.count;
				int countLeft = mSnakeView.getCountLeft();
				int countRight = mSnakeView.getCountRight();
				int countUp = mSnakeView.getCountUp();
				int countDown = mSnakeView.getCountDown();

				db.storeData(countLeft, countRight, countUp, countDown, countBack, count);
	...
	private BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (bundle != null) {
				...
				} else if ("back".equalsIgnoreCase(data)) {
					onBackPressed();
				}
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_SETTINGS_ID:
			...
			if (mSnakeView.noSmallSize) {
				RadioButton smsrg3r2 = (RadioButton) layouts
						.findViewById(R.id.smsrg3r2);
					...
					.setPositiveButton(res.getString(R.string.dlg_yes),
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
									...
									db.storeData(countLeft, countRight,
											countUp, countDown, countBack, count);
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		...
		case DIALOG_SETTINGS_ID:
			...
			break;
		case DIALOG_RECORDS_ID:
			TextView mTView = (TextView) dialog
					.findViewById(R.id.records_layout_text);
	...
	public void onBackPressed() {
		...
		if (mSnakeView.getMode() == SnakeView.RUNNING) {
			...
			db.storePlayTime(gameTime, diff);
			Log.i("saving Time", gameTime + "," + diff);
			db.storeData(mSnakeView.getCountLeft(), mSnakeView.getCountRight(),
					mSnakeView.getCountUp(), mSnakeView.getCountDown(),
					countBack, SnakeView.count);
			Log.i("saving count", "saving count");
